...
namespace Sel.TestAuto
{
    public static class GenericMethods
    {
        ...
        {
            //AutomationCore ac = new AutomationCore();
            string testn = AutomationCore.testName;
            try
            {
                //if(testName=="")
                //{
                ...
                //else
                //{
                    ((ITakesScreenshot)Browsers.GetDriver).GetScreenshot().SaveAsFile(AutomationCore.reportFolder + "\\" + "Capture_" + testn + "_" + DateTime.Now.Second.ToString() + ".png", ScreenshotImageFormat.Png);
                //}
        ...
        //Overload for Click() with Javascript executor
        ...
        public static bool SelectValueFromSlideDropDown(IWebElement SelectionObject, string menu, IWebElement divDropDown, string value)
        {
            ...
            try
            {
                ...
                if (val_act.ToLower().Contains(menu.ToLower()))
                {
                    ...
                    action.MoveToElement(SelectionObject).Perform();
                    //SelectionObject.Click();
                    Thread.Sleep(3000);
        ...
        public static bool SelectValueFromSlideDropDown(IWebElement SelectionObject, string menu, IWebElement divDropDown, string value, string subValue, string subsubValue="")
        {
            Boolean flag = false;
            try
            {
                    ...
                    foreach (IWebElement span in dlistVals)
                    {
                        ...
                        if (span.Text.ToLower().Equals(value.ToLower()))
                        {
                            ...
                            if (subVal.Exists(10))
                            {
                                ...
                                if(subsubValue!="")
                                {
                                    IWebElement ssval = span.FindElement(By.XPath(".//following::a/span[text()='"+subsubValue+"']"));
                                    if(ssval.Exists(10))
                                    {
                                        Thread.Sleep(1000);
                                        ssval.Click();
        ...
        //SelectMenuFromDropDown NG Screen
        public static bool SelectValueFromSlideDropDown_NG(string menu, string option, string subOption = null, string fileType=null)
        {   
            By mainMenu = By.XPath(".//div[@id='navbarSupportedContent']//a[@id='navbarDropdownMenuLink' and text()='"+menu+"']");

            bool flag = false;
            try
            {
                if(Browsers.GetDriver.FindElements(mainMenu).Count > 0)
                {
                    var obj_menu = Browsers.GetDriver.FindElement(mainMenu);
                    obj_menu.Highlight();

                    obj_menu.Click();
                    //Actions action = new Actions(Browsers.GetDriver);
                    //action.MoveToElement(obj_menu).Perform();
                    Thread.Sleep(3000);
                    
                    if(obj_menu.FindElements(By.XPath(".//parent::*//ul[@class='dropdown-menu show']//li")).Count > 0)
                    {
                        if(obj_menu.FindElements(By.XPath(".//parent::*//ul[@class='dropdown-menu show']//a[text()='"+option+"']")).Count > 0)
                        {
                            var opt = obj_menu.FindElement(By.XPath(".//parent::*//ul[@class='dropdown-menu show']//a[text()='" + option + "']"));
                            opt.Click();
                            flag = true;

                            if(subOption!=null)
                            {
                                if(opt.FindElements(By.XPath(".//parent::li//li//a[text()='" + subOption + "']")).Count > 0)
                                {
                                    var subopt = opt.FindElement(By.XPath(".//parent::li//li//a[text()='" + subOption + "']"));
                                    subopt.Click();
                                    flag = true;
                                    if (fileType!=null)
                                    {
                                        if(subopt.FindElements(By.XPath(".//following::a[contains(text(),'"+fileType+"')]")).Count > 0)
                                        {
                                            subopt.FindElement(By.XPath(".//following::a[contains(text(),'" + fileType + "')]")).Click();
                                            flag = true;
                                        }
                                        else
                                        {
                        ...
                        {
                            Assert.Fail("Unable to find the option : " + menu + ">" + option);
                        }
                    }
                    else if (obj_menu.FindElements(By.XPath(".//parent::*//div//a")).Count > 0)
                    {
                        if (obj_menu.FindElements(By.XPath(".//parent::*//div//a[text()='" + option + "']")).Count > 0)
                        {
                            obj_menu.FindElement(By.XPath(".//parent::*//div//a[text()='" + option + "']")).Click();
                            flag = true;
                        }
                        else
                        {
                    ...
                    {
                        Assert.Fail("Unable to find any option under menu : " + menu);
                    }
                }
                else
                {
                    Assert.Fail("Unable to find Menu : " + menu);
                }
        ...
        //Enter text box values in Form
        public static bool EnterTextInForm(string label, string value)
        {
            bool flag = false;
            //.//span[contains(@class,'label')]//span[text()='Paycode']//following::input
            try
            {
                By eleText = By.XPath(".//span[contains(@class,'label')]//span[text()='" + label + "']//following::input");
                if (Browsers.GetDriver.FindElement(eleText).Exists())
                {
                    Browsers.GetDriver.FindElement(eleText).SetText(value);
                    Thread.Sleep(1000);
                    flag = true;
                }
            }
        ...
        //Check Uncheck values in Form
        public static bool CheckUncheckInForm(string label, bool check)
        {
            bool flag = false;
            //.//span[contains(@class,'label')]//span[text()='Paycode']//following::input
            try
            {
                ...
                if (Browsers.GetDriver.FindElement(eleText).Exists())
                {
                    var actVal = Browsers.GetDriver.FindElement(eleText).GetProperty("checked");
                    if (actVal.ToLower() == "true")
                    {
                        if (!check)
                        {
                            Browsers.GetDriver.FindElement(eleText).ButtonClick();
                            Thread.Sleep(1000);
                            flag = true;
                        }
                    }
        ...
        //Select Dropdown values in form
        public static bool SelectDropDownInForm(string label, string value)
        {
            bool flag = false;
            try
            {
                ...
                if (Browsers.GetDriver.FindElement(eleText).Exists())
                {
                    Browsers.GetDriver.FindElement(eleText).SelectValueFromDropDown(value);
                    Thread.Sleep(1000);
                    flag = true;
                }
            }
        ...
        //Get text values in Form
        public static string GetLabelTextValueInForm(string label)
        {
            try
            {
                By eleText = By.XPath(".//span[contains(@class,'label')]//span[text()='" + label + "']");
                if (Browsers.GetDriver.FindElement(eleText).Exists())
                {
                    return Browsers.GetDriver.FindElement(eleText).FindElements(By.XPath(".//following::span[contains(@class,'field')]//span")).FirstOrDefault().Text;
                }
                else
                {
                    return "";
        ...
        //Select Date from Calendar pop up in Form
        public static bool SelectDateValueInForm(string label, string date)
        {
            bool flag = false;
            try
            {
                By eleText = By.XPath(".//span[contains(@class,'label')]//span[text()='" + label + "']//following::span[contains(@class,'field')]//*[@class='rcCalPopup']");
                if (Browsers.GetDriver.FindElement(eleText).Exists())
                {
                    flag = Browsers.GetDriver.FindElement(eleText).SelectDateFromCalendarPopup(date);
                    Thread.Sleep(1000);
                }
            }
            catch (Exception ex)
                    ...
                    if (calMain.Exists())
                    {
                        ...
                        else
                        {
                            ...
                            if (driver.FindElements(By.XPath(".//td/a[text()='" + month + "']")).Count > 0 && driver.FindElements(By.XPath(".//td/a[text()='" + year + "']")).Count > 0)
                            {
                                ...
                                //if (driver.FindElements(By.XPath(".//td[@title='" + fullDate + "']/*[text()='" + day + "']")).Count > 0)
                                {
                                    //var selectDate = driver.FindElement(By.XPath(".//td[@title='" + fullDate + "']/*[text()='" + day + "']/parent::td"));
//                                    var dates = driver.FindElements(By.XPath(".//td/*[text()='" + day + "']"));
                                    
                                    var selectDate = driver.FindElements(By.XPath(".//td/*[text()='" + day + "']")).LastOrDefault().FindElement(By.XPath("./parent::td"));

                                    if(selectDate.GetAttribute("class").Contains("rcOtherMonth"))
                                    {
                                        selectDate = driver.FindElements(By.XPath(".//td/*[text()='" + day + "']")).FirstOrDefault().FindElement(By.XPath("./parent::td"));
                                    }

                                    while (selectDate.GetAttribute("class").Contains("rcWeekend") || selectDate.GetAttribute("class").Contains("rcOutOfRange") || selectDate.GetAttribute("class").Contains("rcDisabled"))
                                    {
                                        date = Convert.ToDateTime(date).AddDays(1).ToString("M/d/yyyy");
                                    ...
                                    try
                                    {
                                        if (selectDate.GetAttribute("className").Contains("rcSelected"))
                                        {
                                            selectDate.Click();
                                            Thread.Sleep(1000);
                                        }
                                    ...
                                    {
                                        selectDate.FindElements(By.XPath(".//a")).First().ButtonClick();
                                    }
        ...
        //Open or Upload file from Dialog
        public static bool OpenUploadFileFromDialog(string filePath, string fileName)
        {
            bool flag = false;
            try
            {
                if (AutoItX.WinWaitActive("Open", "", 10) != 0)
                {
                    AutoItX.Send(filePath + "\\" + fileName);
                    SendKeys.SendWait(@"{Enter}");
                    //Thread.Sleep(TimeSpan.FromSeconds(waitTime));
        ...
        //Read a csv file in to DataTable
        // using System.Data;
        // using System.Data.OleDb;
        // using System.Globalization;
        // using System.IO;
